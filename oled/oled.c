...
static void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8| GPIO_Pin_9;	//选择控制的引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;//普通输出模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//100MHz                    
	GPIO_Init(GPIOB,&GPIO_InitStructure);	//调用库函数初始化GPIOB

	OLED_SCLK_Set();	//设PB8（SCL）为高电平
	OLED_SDIN_Set();	//设PB9（SDA）为高电平
}
...
void OLED_Init(void)
{
	...
	Delay_ms(500);	//延迟，由于单片机上电初始化比OLED快，所以必须加上延迟，等待OLED上复位完成
